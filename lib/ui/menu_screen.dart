...
class MenuScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<Card> allLangs = languages.map(
     (e)=>Card(
      child: ListTile(
                  ...
                  child: Padding(
                    ...
                    child: TextField(
                      ...
                      onChanged: (data){
                        ...
                        List<Card> searchedLangs = searched.map(
                            (e)=>Card(child: ListTile(
                              shape: const RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(10))),
                              leading: e.icon ?? FileIcon(".${e.extension}", size: 45),
                              title: Padding(
                                padding: const EdgeInsets.only(left: 5),
            ...
            BlocBuilder<MenuSearchBloc, MenuSearchState>(
              builder: (context, state) {
                return SliverList(
                  ...
                  childCount: state.searchedLangs.isEmpty?languages.length:state.searchedLangs.length,
                  (context,index){
                    return Padding(
                      padding:const EdgeInsets.symmetric(horizontal: 12, vertical: 3),
                      child: state.searchedLangs.isEmpty? allLangs[index]:state.searchedLangs[index],
                      );
                    },
                  ),
                );
